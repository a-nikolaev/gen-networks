...
(* list of chars *)
let list_of_string s = fold_up (fun i acc -> s.[i]::acc) 0 (String.length s) []
...
let scan flags_ls params_ls arr index_start =
  ...
  let rx_not_match = Str.regexp ("a^") in

  (* Regular expressions for 5 cases: *)
  (* I     (1..)    
          -abcde   *)
  let rx_short_flags = 
    if str_short_flags = "" then rx_not_match 
    else Str.regexp (Printf.sprintf "^-\\([%s]+\\)$" str_short_flags) in
  (* II    (1..)2(3..)  
          -abcdeXparam   
          -abcdeX ...    
               -X ...    *)
  let rx_short_flags_param = 
    if str_short_flags = "" || str_short_params = "" then rx_not_match 
    else Str.regexp (Printf.sprintf "^-\\([%s]*\\)\\([%s]\\)\\(.*\\)$" str_short_flags str_short_params) in
  (* III    (1.)  
          --name         *)
  let rx_long_flag = 
    if long_flags = [] then rx_not_match 
    else Str.regexp (Printf.sprintf "^--\\(%s\\)$" (String.concat "\\|" long_flags)) in
  (* IV     (1.) 
          --name ...     
          --name= ...     *)
  let rx_long_param1 = 
    if long_params = [] then rx_not_match 
    else Str.regexp (Printf.sprintf "^--\\(%s\\)=?$" (String.concat "\\|" long_params)) in
  (* V      (1.) (2..) 
          --name=param   *)
  let rx_long_param2 = 
    if long_params = [] then rx_not_match 
