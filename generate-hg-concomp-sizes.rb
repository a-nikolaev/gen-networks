...
def compute_concomp(aa, cc, ll)
    ...
    num.times{
      max_cc = `./growth --type=hg --model=acl:2:#{aa},#{cc},#{ll} --any --n=#{iters}`
...
8.times{
  cc = 1
  [4].each{|ll|
    [0.05, 0.1, 0.12, 0.14, 0.16, 0.18, 0.19, 0.2, 0.21, 0.22, 0.24, 0.26, 0.3, 0.35, 0.4, 0.45, 0.5,
     0.55, 0.6, 0.65, 0.7, 0.8, 0.9].each{|aa|
